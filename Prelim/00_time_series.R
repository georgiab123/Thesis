...
ggplot() +
  ...
  geom_line(data = HenryHub_sub, aes(x = Date, y = Price, color="Henry Hub spot price"), color = "blue") +
  labs(colour="Legend",x="Date",y="Dollars per one Million British Thermal Units (MMBTU)") +
  geom_vline(xintercept=as.numeric(HenryHub_sub$Date[198]), linetype=3, color = "black")



# data analysis ###############################################################
ccf(Dutch_TTF$Close,HenryHub_sub$Price)
print(ccf(Dutch_TTF$Close,HenryHub_sub$Price))



# checking if time series is stationary using Dickey-Fuller Test:
# our time series are non-stationary

adf.test(HenryHub$Price)
adf.test(HenryHub_sub$Price)
